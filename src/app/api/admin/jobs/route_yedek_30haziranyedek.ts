...
export async function GET() {
  ...
  try {
    ...
    // Ders bazında tekilleştirme: Her ders için sadece en güncel işi kuyruk listesinde gösterir
    const uniqueJobsMap = new Map();
    for (const job of rawJobs) {
      if (!uniqueJobsMap.has(job.courseSlug)) {
        uniqueJobsMap.set(job.courseSlug, job);
      }
    }
