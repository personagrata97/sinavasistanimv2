  ...
  for (let p = 0; p < Math.min(25, pageTexts.length); p++) {
    ...
    for (const rawLine of pageTexts[p].split("\n")) {
      ...
      const normalizedLine = line
        ...
        .replace(/\s+/g, " ")
        .trim()
      if (normalizedLine.length >= 4 && normalizedLine.length <= 200) {
    ...
    for (const rawLine of pageTexts[p].split("\n")) {
      ...
      const normalizedLine = line
        ...
        .replace(/\s+/g, " ")
        .trim()
      if (normalizedLine.length < 4 || normalizedLine.length > 200) continue
      ...
      for (const pattern of tocLinePatterns) {
        ...
        if (/^\d+\.\d+/.test(title)) continue // Alt başlıkları (1.1, 4.2 vb.) eliyoruz
...
function titleMatchesAtPage(title: string, pageText: string, headerLines = 15): boolean {
  ...
  for (const line of lines) {
    ...
    // Bir satırın başlık kabul edilebilmesi için o satırın uzunluğunun,
    // aranan başlık uzunluğundan çok fazla olmaması gerekir (paragraf/cümle içinde geçişi önlemek için).
    if (cleanLine.length > cleanTitleStr.length + 30) continue

    if (fuzzyIncludes(cleanLine, cleanTitleStr)) return true
    if (strippedNum.length >= 4 && fuzzyIncludes(cleanLine, strippedNum)) return true
    if (numMatch && (fuzzyIncludes(cleanLine, `${num}${rest}`) || fuzzyIncludes(cleanLine, rest))) return true
    if (upperTitle.length >= 5 && fuzzyIncludes(cleanLine, upperTitle)) return true
  }
